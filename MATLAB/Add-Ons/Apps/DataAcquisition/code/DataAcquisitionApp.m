classdef DataAcquisitionApp < handle
    %
    % Usage:
    %     app.DataAcquisitionApp
    ...
    properties (Constant)
        AppPathBase = {'/DataAcquisition/code'};
        AppClass = 'DataAcquisitionApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function obj = DataAcquisitionApp()
        ...
        function startApp(obj)
            ...
            mlock;
            DataAcquisitionApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'DAQmain')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong DAQmain
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'DAQmain').getString, ...
            ...
            if nargout(@DAQmain) == 0
                eval('DAQmain');
            else
                obj.AppHandle = eval('DAQmain');
            end
